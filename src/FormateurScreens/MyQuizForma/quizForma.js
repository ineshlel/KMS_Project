...
import Animbutton from '../../MyQuiz/animButton';
  ...
  export default class QuizForma extends Component {
    constructor(props){
      super(props);
      this.qno = 0
      this.score = 0
    ...
    render() {
      ...
      const options = Object.keys(currentOptions).map( function(k) {
        return (  <View key={k} style={{margin:8}}>
   
         <Animbutton countCheck={_this.state.countCheck} onColor={COLORS.orange} effect={"tada"} _onPress={(status) => _this._answer(status,k)} text={currentOptions[k]} />
   
        </View>)
      });
   
      return (
        <ScrollView style={{backgroundColor: '#F5FCFF',paddingTop: 25}}>
        ...
        {/*   <Button
          ...
          <TouchableOpacity onPress={() => this.prev()} >
            <View style={{marginTop:15, marginHorizontal:10,paddingTop: 3,paddingBottom: 3, paddingRight: 20, paddingLeft: 20, borderRadius:10, backgroundColor:COLORS.blueClair}}>
